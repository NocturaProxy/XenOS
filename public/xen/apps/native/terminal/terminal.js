...
class XenTerm {
  ...
  async #setupFS() {
    if (!this.dfs)
      this.dfs = await window.top.xen.fs.openDir("/xen/users/guest/");
    if (!this.fs)
      this._fs = await window.top.xen.fs.openDir(
        "/xen/system/apps/Xen/terminal",
      );

    if (!(await this._fs.exists("lib"))) await this._fs.mkdir("lib");

    this.fs = await this._fs.openDir("lib");

    if (!(await this.fs.exists("cmd"))) await this.fs.mkdir("cmd");
  ...
  async activate(term) {
    ...
    this.term.writeln(c.bold(await this.fs.readFile("motd.txt", "utf-8")));
      ...
      "\r\n" +
        ...
        `: ${
          this.dir == this.baseDir
            ? "~"
            : top.path
                .dirname(this.dir.replace(/\/?$/, "/jbjbk"))
                .split("/")
                .pop() || "/"
        } $ ` +
        text,
  ...
  parseCommand(command) {
    const args = command
      .match(/(?:(?:"[^"]+")|(?:[^\s]+))/gim)
      .map((match) => (match.startsWith('"') ? match.slice(1, -1) : match));
    ...
    if (event.key === "Enter") {
      ...
      if (this.typing.length > 0) {
        const history = JSON.parse(
          await this.fs.readFile("history.txt", "utf-8"),
        );
        ...
        if (command === "clear") {
          ...
          return import("/xen/~/terminal/commands/" + command + ".js")
            .then(async ({ help, run }) => {
              return run(args, {
                fs: this.dfs,
                term: this.term,
...
[new WebLinksAddon.WebLinksAddon(), fit, new XenTerm(term)].forEach(
  term.loadAddon.bind(term),
);
