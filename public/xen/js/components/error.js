const errorManager = {
    ...
    async init() {
        this.restartBtn.addEventListener('click', () => {
            ...
            this.restartBtn.querySelector(".crashSpinner").style.visibility = "visible";
        ...
        this.tsBtn.addEventListener('click', async () => {
            ...
            if (window.xen.apps) window.xen.apps.processes.forEach(
                process => {
                    if (process.worker) {
    ...
    async startup(err) {
        ...
        this.errorContainer.querySelector("h2").innerText = `Fatal Error: ${err.name}`;
        this.errorContainer.querySelector("p").innerHTML = `${err.message}<br><br><details>
