// slightly modified https://www.npmjs.com/package/pure-context-menu because im lazy
...
class PureContextMenu {
  ...
  oncontextmenu = (event) => {
    ...
    const closestMenu = event.target.closest(
      `.${this._options.contextMenuClass}`,
    );
    if (closestMenu) {
      return;
    }
  ...
  _handleItemClick = (event) => {
    ...
    if (li) {
      const index = [...li.parentElement.childNodes].findIndex(
        (item) => item === li,
      );
